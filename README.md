...
- Local storage.  Global coverage requires ~5 Tb which must also be available on Google Drive.  Usage example provided below requires ~15 Gb.
...
## Data Examples. 
We provide a sample of 33 full MGRS zones as examples (samples folder).  These cover the Americas, Africa, Europe, East Asia and North-West Russia.  Once downloaded, the tiles can be directly read in any GIS software. Classes are: 0: background (transparent) 1: Rivers (light blue); 2: Lakes (green); 3: Sediment Bars (red); 4: Oceans (royal blue); 5: Glaciated Terrain (cyan); 6: Snow (light grey); 7: Clouds (dark grey); 8: Data Gaps (black)