...
## Final Outputs
The final class raster for each MGRS zone will be saved as Class_S2_Month_Zone.tif.  For example, the class raster for MGRS zone 53V is saved as Class_S2_Z53V.tif.  It covers longitudes 132 W to 138 W and latitudes 56 to 64 North. It is saved and projected in UTM zone 53 North (EPSG:32653).  The use of a projected coordinate system allows for pixel sizes that are a constant 10 meters, as per the Sentinel-2 data acquisition, for the globe.  The data are highly compressed and are typically 20 to 50 Mb for each MGRS gridzone comprising in excess of 4 Gigapixels.  Global coverage can be achieved in less than 10 Gb.  For each class raster, a color table is saved with the background set to transparent.  Below is an example from MGRS zone 53V with a Google Satellite base image.    

![Screenshot](Example.png)

## Data Examples. 
We provide a sample of 15 full MGRS zones as examples (samples folder).  These are from the Americas, Africa, Europe, East Asia and North-West Russia.  Once downloaded, the tiles can be directly read in any GIS software. Classes are: 0: background (transparent) 1: Rivers (light blue); 2: Lakes (green); 3: Sediment Bars (red); 4: Oceans (royal blue); 5: Glaciated Terrain (cyan); 6: Snow (light grey, unused); 7: Clouds (dark grey); 8: Data Gaps (black)
